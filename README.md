...
> 全部使用现成开发板模块，无需焊接芯片，杜邦线互连，底座固定。

### 物料清单 (BOM)

| 物料 | 型号 | 数量 | 单价(约) | 说明 |
|------|------|:----:|----------|------|
| 主控模块 | ESP32-C3 Mini 开发板 (合宙) | 1 | ~8 元 | 板载 USB、LDO、按键，直接用 |
| 陀螺仪模块 | GY-521 (MPU6050) | 1 | ~4 元 | 模块已含上拉电阻和稳压 |
| 充放电模块 | TP4056 Type-C 模块 (带保护) | 1 | ~2 元 | DW01A+FS8205 一体，带 OUT± |
| 电池 | 3.7V LiPo 电池 (1000mAh) | 1 | ~10 元 | 扁平软包，适合底座内放置 |
| 开关 | SS-12D00 滑动开关 | 1 | ~0.5 元 | 串联在电源通路上 |
| 载板 PCB | 定制 PCB (嘉立创打样) | 1 | ~5 元 | 5片起打，排母座插接各模块 |
| 排母座 | 2.54mm 排母 | 若干 | ~2 元 | 焊在载板上，模块插入 |
| 电池接口 | JST-PH 2.0 座子 | 1 | ~0.5 元 | LiPo 电池可插拔 |
...
电源部分:
                ┌───────────────┐
USB Type-C ───→ │ TP4056 模块    │
                │  IN+ / IN-    │
                │               │
LiPo 电池 ────→ │  BAT+ / BAT-  │
 (3.7V)         │               │
                │  OUT+ ────────┼──→ 滑动开关 ──→ ESP32-C3 VIN (5V)
                │  OUT- ────────┼──→ ESP32-C3 GND
                └───────────────┘

注: TP4056 带保护模块 (DW01A+FS8205), OUT+/OUT- 为保护后输出。
    ESP32-C3 Mini 板载 LDO，VIN 接 3.7V 电池电压即可。
```
...
```
                     TP4056 模块 (带保护)
                  ┌─────────────────────┐
 Type-C ──→ IN+  │  TP4056    DW01A    │  OUT+ ──→ 滑动开关 ──→ ESP32 VIN
        ──→ IN-  │ (充电IC)  (保护IC)  │  OUT- ──→ GND
                  │     ↕               │
                  │   BAT+ ←── LiPo +  │
                  │   BAT- ←── LiPo -  │
                  └─────────────────────┘
...
- **充电**: Type-C 插入 → TP4056 以 CC/CV 方式充电 (默认 1A, 可换电阻调整)
- **放电保护**: DW01A 监测电池电压，低于 2.4V 自动断开输出
- **过充保护**: TP4056 充满 4.2V 自动停止
- **开关**: 滑动开关串联在 OUT+ 和 ESP32 VIN 之间，物理断电
