...
基于 **ESP32-WROOM-32**（ESP32-DevKitC 风格开发板）+ MPU6050 陀螺仪的椅子久坐检测器。安装在椅子坐垫下方，通过检测坐垫倾斜角度变化判断是否有人坐，超时提醒起身活动。内置锂电池充放电，USB 充电，滑动开关控制电源。
...
| 模块 | 状态 |
|------|------|
| 硬件方案 | ✅ 已完成 (ESP32-WROOM-32 + GY-521 + PM11) |
| 载板 PCB 设计 | ✅ 已完成 (接线表、网络表、KiCad 工程) |
| Wokwi 仿真 | ✅ 已完成 (ESP32 + MPU6050, 检测算法验证) |
| 固件开发 | 🔧 联调中 (Mock 模式 + 串口 bring-up) |
| 3D 底座 | ⏳ 待设计 |
| 采购 | ⏳ 待下单 |
...
| 物料 | 型号 | 单价 | 说明 |
|------|------|------|------|
| 主控模块 | ESP32-WROOM-32 开发板 (DevKitC) | ¥35.67 | CP2102 USB 串口, WiFi+BT |
...
| [firmware/README.md](firmware/README.md) | 固件编译、烧录、串口联调 |
...
│   └── src/
│       └── main.cpp             # 仿真固件
├── firmware/                    # ESP32-WROOM-32 固件 (PlatformIO)
│   ├── README.md                # 联调指南
│   ├── FIRMWARE.md              # 架构与算法设计
│   ├── src/
│   │   ├── main.cpp             # 主逻辑 + 串口 bring-up
│   │   ├── detector.h / .cpp    # 久坐检测状态机
│   │   └── config.h             # 引脚与阈值
...
- [x] 购买 ESP32-WROOM-32 开发板（已联调 USB 烧录）
- [ ] 购买 GY-521 + PM11 模块
- [ ] 面包板搭建原型，验证接线
- [ ] 实测椅子坐下/站起时的角度变化数据
- [ ] 确定安装位置和固定方式
