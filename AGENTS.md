...
久坐检测器载板 PCB (KiCad 10)。**ESP32-WROOM-32** + **GY-87** + **OLED** + PM11（载板无按键/滑动开关）。原理图 **v0.6**；PCB **v0.4**（布局与网络已分配，**未布线**——等 03/04 软件联调稳定后再走线打样）。
...
- [x] 模块联调台架：GY-87 → `../04-MCU-GY87-Debug`；OLED → `../03-ESP32-OLED-Button-Demo`（**软件仍在调试**）
- [x] 本仓库已移除 `software/`（固件不在此维护）
- [x] J2 排母与 GY-87 引脚顺序对齐（可直插）
- [ ] PCB 走线：**暂缓**，待 03/04 联调稳定后再布
- [ ] DRC 通过 → Gerber 打样
